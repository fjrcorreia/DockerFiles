...
start_jboss() {
    ...
    /bin/sh -c "JBOSS_PIDFILE=${JBOSS_INIT_PID_FILE} LAUNCH_JBOSS_IN_BACKGROUND=1 ${JBOSS_HOME}/bin/standalone.sh -c ${JBOSS_CONFIG}" &
    sleep 5
...
#
#  - Creates an adminstration user, usefull for remote deploy
